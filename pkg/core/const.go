...
var (
	...
	ptrLLMEndpoint                = flag.String("llm-endpoint", firstNonEmpty(os.Getenv("EARLYBIRD_LLM_ENDPOINT"), "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent"), "OpenAI-compatible chat completions endpoint for LLM scanning")
	ptrLLMModel                   = flag.String("llm-model", firstNonEmpty(os.Getenv("EARLYBIRD_LLM_MODEL"), "gemini-2.5-flash"), "Model to use for LLM scanning")
	...
	ptrLLMFailClosed              = flag.Bool("llm-fail-closed", false, "Fail the scan if an enabled LLM scan request cannot be completed")
